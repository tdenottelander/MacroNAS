#!/bin/bash
if [ $# -eq 0 ]; then
  echo "Please provide the path to the executable as parameter like so: bash run_bench_cifar100.sh [path_to_executable]"
  exit 1
fi
...
netuniqevals=100000
fit="cifar100 14 2" # optimize on 14 cells of the architecture, on 2 objectives
